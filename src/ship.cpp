...
{
	TextureManager::Instance()->load("../Assets/textures/trooper.png", "dude");

	auto size = TextureManager::Instance()->getTextureSize("dude");
	setWidth(size.x);
	setHeight(size.y);

	getTransform()->position = glm::vec2(750.0f, 450.0f);
...
void Ship::draw()
{
	...
	// draw the ship
	TextureManager::Instance()->draw("dude", x, y, m_currentHeading, 255, true);
}
